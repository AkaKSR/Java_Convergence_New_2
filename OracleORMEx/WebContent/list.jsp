...
<body>
		<table border="1">
			...
			<c:if test="${pvo.totalCount>0 }">
				<c:set var="no" value="${pvo.totalCount-(pvo.currentPage-1)*pvo.pageSize }"/>
				<c:forEach var="vo" items="${pvo.list }" varStatus="vs">
					<tr>
						<td><span id="idx${vo.idx }">${no-vs.index }</span></td>
						<td>
			...
			<tr>
				<td align="right" colspan="5" style="border: none;">
					<form action="insertOk.jsp" method="post" id="myForm">
						...
						<div class="row"  style="margin-top: 5px;">
							<div class="col">
								<input type="text" name="memo" id="memo" class="form-control" required="required" placeholder="내용을 입력하세요">
							</div>
						</div>
						<div class="row"  style="margin-top: 5px;">
							<div class="col">
