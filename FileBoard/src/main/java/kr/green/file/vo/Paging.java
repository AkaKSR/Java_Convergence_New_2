...
public class Paging<T> {
	...
	public String getPageList2() {
		...
		if(startPage>1) {
			sb.append("<li class='page-item'>");
			sb.append("<a class='page-link' href='javascript:sendPost(\"?\", {\"p\":" + (startPage-1) + ",\"b\":" + blockSize + ",\"s\":" + pageSize + "});' aria-label='Previous'>");
			sb.append("<span aria-hidden='true'>&laquo;</span>");
			sb.append("<span class='sr-only'>Previous</span>");
			sb.append("</a>");
			sb.append("</li>");
		...
		for(int i=startPage;i<=endPage;i++) {
			...
			if(i==currentPage) {
				...
				sb.append("<li class='page-item'><a class='page-link' href='javascript:sendPost(\"?\", {\"p\":" + (i) + ",\"b\":" + blockSize + ",\"s\":" + pageSize + "});'>" + i + "</a></li>");
			}
		}
		// 다음은 끝페이지 번호가 전체페이지 번호 보다 적을때만 있다.
		if(endPage<totalPage) {
			sb.append("<li class='page-item'>");
			sb.append("<a class='page-link' href='javascript:sendPost(\"?\", {\"p\":" + (endPage+1) + ",\"b\":" + blockSize + ",\"s\":" + pageSize + "});' aria-label='Next'>");
			sb.append("<span aria-hidden='true'>&raquo;</span>");
			sb.append("<span class='sr-only'>Next</span>");
			sb.append("</a>");
			sb.append("</li>");
